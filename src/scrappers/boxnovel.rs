...
pub fn get_novel(input: &str) -> Novel {
    ...
    for link in chapter_links {
        chapters.push(get_chapter(&Url::parse(&link).unwrap()));
...
pub fn get_chapter(url: &Url) -> Chapter {
    let body = get_response(url);
...
pub fn get_response(url: &Url) -> String {
    get(url.to_string()).unwrap().text().unwrap()
}
...
pub fn fetch_chapter_links(url: &Url) -> Vec<String> {
    let new_url = url.join("ajax/chapters").unwrap().to_string(); //vec![url, "ajax/chapters"].join("");
